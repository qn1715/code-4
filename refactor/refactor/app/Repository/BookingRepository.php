...
class BookingRepository extends BaseRepository {
	...
	/**
	 * This function is used to get the jobs history related to a user
	 * 
	 * @param $user_id
	 * @return array
	 */
	public function getUsersJobsHistory($user_id, Request $request) {
		...
		if ($cuser->is('customer')) {
			...
			$response['emergencyJobs'] 	= $emergencyJobs;
			$response['noramlJobs'] 	= [];
			$response['jobs'] 			= $jobs;
			$response['cuser'] 			= $cuser;
	...
	public function store($user, $data) {
		...
		if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
			...
			if (!isset($data['from_language_id']) || empty($data['from_language_id'])) {
	...
	/**
	 * This function is used to save already store job emails
	 * 
	 * @param $data
	 * @return mixed
	 */
	public function storeJobEmail($data) {
		...
		if(!$job->save()) {
			$response['message'] = 'Some thing went wrong, while during job save';
			return $response;
		}
	...
	public function updateJob($id, $data, $cuser) {
		...
		if ($job->due <= Carbon::now()) {
			...
			if($job->save()) {
				...
				$response['message'] = 'Job Update successfully.';
			} else {
				$response['message'] = 'Something went wrong, while during job updation.';
			}
			return $response;
		} else {
			if($job->save()) {
...
//        }
	...
	private function changeTranslator($current_translator, $data, $job) {
		...
		if (!is_null($current_translator) || (isset($data['translator']) && $data['translator'] != 0) || !empty($data['translator_email'])) {
			...
			if (!is_null($current_translator) && ((isset($data['translator']) && $current_translator->user_id != $data['translator']) || !empty($data['translator_email'])) && (isset($data['translator']) && $data['translator'] != 0)) {
        ...
        */
		...
		if ($cuser->is('customer')) {
			...
			if ($translator) {
				...
				$language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
				$msg_text = ["en" => 'Kunden har avbokat bokningen för ' . $language . 'tolk, ' . $job->duration . 'min, ' . $job->due . '. Var god och kolla dina tidigare bokningar för detaljer.'];
			...
			if ($job->due->diffInHours(Carbon::now()) > 24) {
				...
				if ($customer) {
					...
					$data['notification_type'] = 'job_cancelled';
					$language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
					$msg_text =  ["en" => 'Er ' . $language . 'tolk, ' . $job->duration . 'min ' . $job->due . ', har avbokat tolkningen. Vi letar nu efter en ny tolk som kan ersätta denne. Tack.'];
	...
	/**
	 * [bookingExpireNoAccepted description]
	 * @return [array]
	 */
	...
	/**
	 * This function is used to reopen and create a new job for specific user 
